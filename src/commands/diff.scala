...
def cmdDiff(args: List[String], ctx: CommandContext): CmdResult =
  requireArg(args, "Usage: scalex diff <git-ref> (e.g. scalex diff HEAD~1)") { ref =>
    ...
    changedFiles.take(ctx.output.limit * 5).foreach { relPath =>
