...
def cmdImpl(args: List[String], ctx: CommandContext): CmdResult =
  requireArg(args, "Usage: scalex impl <trait>") { symbol =>
    ...
    if (results.isEmpty)
      CmdResult.NotFound(s"""No implementations of "$symbol" found""", mkNotFoundWithSuggestions(symbol, ctx, "impl"))
    else
      CmdResult.SymbolList(header = s"""Implementations of "$symbol" — ${results.size} found:""", symbols = results)
  }
