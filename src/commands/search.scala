...
def cmdSearch(args: List[String], ctx: CommandContext): CmdResult =
  requireArg(args, "Usage: scalex search <query>") { query =>
    ...
    ctx.search.returnsFilter.foreach { rt =>
    ...
    ctx.search.takesFilter.foreach { tt =>
      results = results.filter { s =>
        val sig = s.signature
        sig.nonEmpty && {
    ...
    if (results.isEmpty)
      CmdResult.NotFound(s"""Found 0 symbols matching "$query"""", mkNotFoundWithSuggestions(query, ctx, "search"))
    else
      CmdResult.SymbolList(header = s"""Found ${results.size} symbols matching "$query":""", symbols = results)
  }
