...
val KindFlag =
  Flag.string("--kind")("K", "Filter by kind: class, trait, object, def, val, type, enum, given, extension")
val VerboseFlag = Flag.boolean("--verbose")("Show signatures and extends clauses")
val CategorizeFlag =
  Flag.boolean("--categorize", "-c")("Group refs by category (default; kept for backwards compatibility)")
...
val CategoryFlag = Flag.string("--category")(
  "CAT",
  "Refs: filter to a single category (Definition/ExtendedBy/ImportedBy/UsedAsType/Usage/Comment)"
)
...
val PathFlag =
  Flag.string("--path")("PREFIX", "Restrict results to files under PREFIX (e.g. compiler/src/)", _.stripPrefix("/"))
val ExcludePathFlag = Flag.string("--exclude-path")(
  "PREFIX",
  "Exclude files under PREFIX (e.g. --exclude-path sbt-test/)",
  _.stripPrefix("/")
)
...
val MaxLinesFlag =
  Flag.int("--max-lines")("N", default = 0, "Members/overrides/explain: only inline bodies ≤ N lines (0 = unlimited)")
...
val MembersLimitFlag =
  Flag.int("--members-limit")("N", default = 10, "Explain: max members to show per type (default: 10)")
val ExpandFlag = Flag.optionalInt("--expand")(
  "N",
  default = 0,
  presentValue = 1,
  "Explain: recursively expand implementations N levels deep"
)
...
val DepthFlag = Flag.int("--depth")(
  "N",
  default = -1,
  "Hierarchy/deps: max tree depth (hierarchy default: 5, no cap; deps default: 1, max: 5)"
)
...
val ConciseFlag =
  Flag.boolean("--concise")("Overview: fixed-size summary (~60 lines) with top packages, hub types, dep stats")
...
val MaxOutputFlag = Flag.int("--max-output")(
  "N",
  default = 0,
  "Limit result output to N characters (0 = unlimited); truncation metadata is exempt; JSON stays valid"
)
...
val scalexFlags: FlagRegistry = FlagRegistry(
  List(
    WorkspaceFlag,
    LimitFlag,
    OffsetFlag,
...
/** The single place a CommandContext is built from parsed flags — derived values (limit 0 → unlimited, --flat
  * inversion, --exact/--prefix and --up/--down pairing) are resolved here.
  */
def flagsToContext(
    f: Flags,
    idx: WorkspaceIndex,
    ...
    effectiveNoTests: Option[Boolean] = None
): CommandContext = {
  val sawUp = f(UpFlag)
  ...
  CommandContext(
    ...
    output = OutputOptions(
      limit = if (f(LimitFlag) == 0) Int.MaxValue else f(LimitFlag),
    ...
    search = SearchOptions(
      ...
      searchMode = if (f(ExactFlag)) Some("exact") else if (f(PrefixFlag)) Some("prefix") else None,
    ...
    references = ReferencesOptions(categorize = !f(FlatFlag), categoryFilter = f(CategoryFlag), topN = f(TopFlag)),
