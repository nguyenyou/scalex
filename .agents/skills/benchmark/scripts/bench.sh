...
if [ "$MODE" != memory ] && ! command -v hyperfine &>/dev/null; then
  echo "Error: hyperfine not found. Install with: brew install hyperfine"
  exit 1
fi

if [ "$MODE" != memory ] && [ ! -x "$SCALEX_BIN" ]; then
...
run_memory() {
  ...
  SCALEX_JAVA_OPTS="-Xlog:gc*=info:file=$HEAP_LOG" \
    "$PROJECT_ROOT/mill" --no-daemon run --timings index "$SCALA3_DIR" 2>&1
  echo ""
  ...
  > "$HEAP_LOG"
  SCALEX_JAVA_OPTS="-Xlog:gc*=info:file=$HEAP_LOG" \
    "$PROJECT_ROOT/mill" --no-daemon run --timings refs "$SCALA3_DIR" Phase 2>&1
  echo ""
...
run_timings() {
  ...
  echo "--- Cold index ---"
  rm -rf "$SCALA3_DIR/.scalex"
  "$SCALEX_BIN" index "$SCALA3_DIR" --timings 2>&1
  echo ""
  echo "--- Warm index ---"
  "$SCALEX_BIN" index "$SCALA3_DIR" --timings 2>&1
  echo ""
  echo "--- refs Compiler ---"
  "$SCALEX_BIN" refs "$SCALA3_DIR" Compiler --timings 2>&1
  echo ""
}
