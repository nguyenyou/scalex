...
case class EdgeSegment(start: Point, direction: Direction, finish: Point) extends HasRegion {
  def region =
    if (start.column < finish.column || start.row < finish.row) Region(start, finish)
...
class Grid(dimension: Dimension) {
  ...
  override def toString = chars.map(String(_)).mkString("\n")
}

// ── EdgeTracker ─────────────────────────────────────────────────────────────

class EdgeTracker(drawing: Drawing) {
  private val horizontalQuadTree: QuadTree[Region] = QuadTree(drawing.dimension)
  private val verticalQuadTree: QuadTree[Region] = QuadTree(drawing.dimension)
  ...
  private val arrowRegions: List[Region] = drawing.edgeElements.map { edgeElement =>
    if (edgeElement.hasArrow1) edgeElement.startPoint.region
  ...
  for {
    ...
    if (segment.direction.isHorizontal) horizontalQuadTree.add(segment.region)
...
object KinkRemover {
  def removeKinks(drawing: Drawing): Drawing = {
    val edgeTracker = EdgeTracker(drawing)
  ...
  private def removeRows(drawing: Drawing, fromRow: Int, toRow: Int): Drawing = {
    ...
    val newElements = drawing.elements.map {
      case ede: EdgeDrawingElement =>
        ede.copy(bendPoints = ede.bendPoints.map(p => if (p.row >= fromRow) p.up(upShift) else p))
      case vde: VertexDrawingElement =>
        if (vde.region.topRow < fromRow) vde
        else vde.up(upShift)
    }
...
class Renderer(prefs: LayoutPrefs) {
  ...
  private def drawLine(grid: Grid, point1: Point, direction: Direction, point2: Point): Unit = {
    ...
    if (point1 != point2) drawLine(grid, point1.go(direction), direction, point2)
  }

  private def renderEdge(grid: Grid, element: EdgeDrawingElement, drawing: Drawing): Unit = {
    for ((previousSegmentOpt, segment @ EdgeSegment(point1, direction, point2)) <- withPrevious(element.segments)) {
    ...
    def drawBoxIntersection(intersectionPoint: Point, direction: Direction) =
      if (prefs.unicode && drawing.vertexElementAt(intersectionPoint).isDefined && grid.contains(intersectionPoint))
    ...
    for (EdgeSegment(point, direction, _) <- element.segments.headOption)
      if (element.hasArrow1) grid(point) = arrow(direction.opposite)
    ...
    for (EdgeSegment(_, direction, point) <- element.segments.lastOption)
      if (element.hasArrow2) grid(point) = arrow(direction)
  ...
  private def renderVertex(grid: Grid, element: VertexDrawingElement): Unit = {
    ...
    if (prefs.unicode)
      for (row <- (region.topRow + 1).to(region.bottomRow - 1)) {
        ...
        if (grid(left.right) == '\u2500') grid(left) = joinChars(3)
        val right = Point(row, region.rightColumn)
        if (grid(right.left) == '\u2500') grid(right) = joinChars(2)
      }
  }

  private def verticalLineChar = if (prefs.unicode) '\u2502' else '|' // '│'
  private def horizontalLineChar = if (prefs.unicode) '\u2500' else '-' // '─'
  private def intersectionChar = if (prefs.unicode) '\u253c' else '-' // '┼'

  // Indexed glyph sets; call sites map junction kinds to indexes 0-3
  private val bendChars
      : String = // up\u2192right/left\u2192down, up\u2192left/right\u2192down, down\u2192right/left\u2192up, down\u2192left/right\u2192up
    if (prefs.unicode)
      (if (prefs.rounded) "\u256d\u256e\u2570\u256f" else "\u250c\u2510\u2514\u2518") // "╭╮╰╯" or "┌┐└┘"
    else if (prefs.explicitAsciiBends) "/\\\\/"
    else "----"

  private val cornerChars: String = // top-left, top-right, bottom-left, bottom-right
    if (!prefs.unicode) "++++"
    else if (prefs.doubleVertices) "\u2554\u2557\u255a\u255d" // "╔╗╚╝"
    else if (prefs.rounded) "\u256d\u256e\u2570\u256f" // "╭╮╰╯"
    else "\u250c\u2510\u2514\u2518" // "┌┐└┘"

  private val joinChars: String = // edge enters box: from above, below, right, left
    if (prefs.doubleVertices) "\u2564\u2567\u2562\u255f" else "\u252c\u2534\u2524\u251c" // "╤╧╢╟" or "┬┴┤├"

  private def boxHorizontalChar =
    if (prefs.unicode) (if (prefs.doubleVertices) '\u2550' else '\u2500') // '═' or '─'
    else '-'
