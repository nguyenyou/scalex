---
name: scalex
description: "Scala code intelligence CLI for navigating Scala 2/3 codebases. Use when working with .scala files to find where a class/trait/object is defined, who extends a trait, who uses a symbol, what members a class has, reading scaladoc, searching files by name, or searching file contents. Trigger on tasks like: 'where is X defined', 'who implements Y', 'find 
  usages of Z', 'what methods does X have', 'show source of X', 'what's the inheritance tree', 'find types that extend X', or before renaming/refactoring. Also for test navigation: 'what tests exist', 'is X tested', 'find tests for Z'. Use proactively when exploring unfamiliar Scala code — faster and more structured than grep. Supports fuzzy camelCase search (e.g. 'hms' 
  finds HttpMessageService). Always prefer scalex over grep/glob for Scala symbol lookups. Use `scalex grep` instead of the Grep tool for searching inside .scala files."
---
