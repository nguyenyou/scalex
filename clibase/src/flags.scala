...
/** One CLI flag. Instances are singletons declared by the application and used as typed keys into [[Flags]].
  */
sealed trait Flag[A] {
...
object Flag {
  ...
  /** Int-valued flag: `--limit N`. Always consumes the next token; a missing or non-integer value falls back to
    * `default`.
    */
  final class IntFlag private[Flag] (val names: List[String], val label: String, val help: String, val default: Int)
      extends Flag[Int]

  /** Int flag whose value is optional: bare `--expand` means `presentValue`, `--expand 2` means 2. Only consumes the
    * next token when it parses as an integer, so a following flag is never swallowed.
    */
  final class OptionalIntFlag private[Flag] (
  ...
  final class IntOptionFlag private[Flag] (val names: List[String], val label: String, val help: String)
      extends Flag[Option[Int]] {
    val default: Option[Int] = None
  }

  /** String-valued flag: `--kind K`. Always consumes the next token; `transform` is applied to the raw value (e.g.
    * strip a leading `/`).
    */
  final class StringFlag private[Flag] (
      val names: List[String],
      val label: String,
      val help: String,
      val transform: String => String
  ) extends Flag[Option[String]] {
    val default: Option[String] = None
  }

  /** Repeatable string flag: `-e PAT -e PAT`. Values accumulate in order; a value starting with `-` is consumed but
    * dropped, so a following flag is never treated as a pattern.
    */
  final class RepeatedFlag private[Flag] (val names: List[String], val label: String, val help: String)
      extends Flag[List[String]] {
    val default: List[String] = Nil
  }

  def boolean(names: String*)(help: String): BooleanFlag =
    BooleanFlag(names.toList, help)

  def int(names: String*)(label: String, default: Int, help: String): IntFlag =
    IntFlag(names.toList, label, help, default)

  def optionalInt(names: String*)(label: String, default: Int, presentValue: Int, help: String): OptionalIntFlag =
    OptionalIntFlag(names.toList, label, help, default, presentValue)

  def intOption(names: String*)(label: String, help: String): IntOptionFlag =
    IntOptionFlag(names.toList, label, help)

  def string(names: String*)(label: String, help: String, transform: String => String = identity): StringFlag =
    StringFlag(names.toList, label, help, transform)

  def repeated(names: String*)(label: String, help: String): RepeatedFlag =
    RepeatedFlag(names.toList, label, help)
}
  ...
  def optionsHelp: String = {
    flags
      .map { f =>
        ...
        val display = if (f.label.isEmpty) names else s"$names ${f.label}"
...
final class Flags private (values: Map[Flag[?], Any], val positional: List[String]) {
  ...
  def updated[A](flag: Flag[A], value: A): Flags = Flags(values.updated(flag, value), positional)
}

object Flags {

  /** Single left-to-right pass over the arg list. Each token is classified exactly once as a flag, a flag's value, or a
    * positional arg — so a positional that happens to equal a flag's value elsewhere in the list is never
    * misclassified.
    *
    * Lenient by design: unknown `--long` flags are ignored (a caller passing a wrong flag still gets useful output);
    * unknown short tokens stay positional. When a flag appears more than once, the last occurrence wins.
    */
  def parse(args: List[String], registry: FlagRegistry): Flags = {
    ...
    Flags(values, positional.result())
  }
}
